...
if (isset($_FILES['upfile']) && $_FILES['upfile']['error'] ==  0) {
    ...
    if (is_uploaded_file($tempPathName)) {
        if (move_uploaded_file($tempPathName, $fileNameToSave)) {
            ...
            //sql作成＆実行
            $sql = 'UPDATE man_table SET name = :name, icon = :image,area = :area, job = :job, age = :age, height = :tall, physique = :body, updated_at=sysdate() WHERE id = :id';
    ...
    //sql作成＆実行
    $sql = 'UPDATE man_table SET name = :name, area = :area, job = :job, age = :age, height = :tall, physique = :body, updated_at=sysdate() WHERE id = :id';
